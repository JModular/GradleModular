...
public class SocketClientThread extends Thread implements SocketCloseInterface {
    ...
    private void initSocket() {
        try {
            ...
            mSocketReceiveThread = new SocketReceiveThread("SocketReceiveThread",
                    new BufferedReader(new InputStreamReader(mSocket.getInputStream(), "gbk")),
                    socketClientResponseInterface, this);
            mSocketReceiveThread.start();

            //开启发送线程
            PrintWriter printWriter = new PrintWriter(new OutputStreamWriter(mSocket.getOutputStream(), "gbk"), true);
            System.out.println(TAG+ ":initSocket: " + printWriter);
    ...
    public synchronized void stopThread() {
        ...
        if (isReConnect) {
            ...
            initSocket();
            System.out.println(TAG+"stopThread: " + Thread.currentThread().getName());
        }
    }
