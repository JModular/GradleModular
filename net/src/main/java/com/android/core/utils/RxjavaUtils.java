...
public class RxjavaUtils {
	...
	private static void doOnNext() {
		...
		}).subscribeOn(getNamedScheduler("线程1")).doOnNext(new Action1<Integer>() {
			@Override
	...
	public static void observeOn() {
		Observable.just("RxJava").observeOn(getNamedScheduler("map之前的observeOn")).map(s -> {
			threadInfo(".map()-1");
			return s + "-map1";
		}).map(s -> {
			threadInfo(".map()-2");
			...
			threadInfo(".map()-3");
			return s + "-自定义线程任务";
		}).observeOn(getNamedScheduler("subscribe之前的observeOn")).subscribe(s -> {
			threadInfo(".onNext()");
